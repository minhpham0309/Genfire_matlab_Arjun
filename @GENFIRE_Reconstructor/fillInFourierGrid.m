...
if doCTFcorrection
    ...
    nk = floor(dimk1)+1;
    [YY,XX] = meshgrid((1:dimk1)-nk,(1:dimk2)-nk);
    XX=XX(:); YY=YY(:);
    index_r = ones(1,dimk1^2);
    for projNum = 1:size(projections,3);        
...
masterDistances(index_exact)=0;
index_k = masterInd(index_exact);
k_exact = masterVals(index_exact);

%index_unique = unique(index_k);
%acc_exact = accumarray(index_k,k_exact,[dim1^3 1],@mean);
%k_exact_unique = acc_exact(index_unique);
...
%obj.measuredK(index_unique) = k_exact_unique;
%nnz(obj.measuredK)
