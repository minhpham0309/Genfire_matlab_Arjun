...
save('Ni_Pt_DFT_kmeasured.mat','measuredX1','measuredY1','measuredZ1','master_ind','master_dist','master_val')
%}
%% use FFT
%
%l1=l2; k1=k2;
...
sigma = obj.sigma;
s_size = 20;
rangeX = s_size/2:s_size:(l1-nc)
remainder = l1-nc - rangeX(end)
if remainder>s_size/2, rangeX = [rangeX,l1-nc];
else rangeX(end) = l1-nc;
end
...
%}
for t=1:(lenX-1)^3
    if mod(t,(lenX-1)^2)==1, fprintf('\n   %d\n',ceil(t/(lenX-1)^2));end    
    
    [i,j,k] = ind2sub([lenX-1, lenX-1, lenX-1],t);
    ...
    index_ijk = find((measuredX>=rangeX(i) & measuredX<=rangeX(i+1) & measuredZ>=rangeX(j) & measuredZ<=rangeX(j+1) & measuredY>=rangeX(k) & measuredY<=rangeX(k+1)));
   
    if mod(t,lenX-1)==1,fprintf('%d.length = ',j);end
    if ~isempty(index_ijk)
        ...
        goodInd = ~(tmpX_i>l2-nc| tmpX_i<1-nc| tmpY_i>l2-nc| tmpY_i<1-nc| tmpZ_i>l2-nc| tmpZ_i<1-nc) & dist<cutoff_dist;
        tmpX_i = tmpX_i(goodInd);
        tmpY_i = tmpY_i(goodInd);
        tmpZ_i = tmpZ_i(goodInd);
        masterInd_i = sub2ind([l2 l2 l2],tmpX_i+nc,tmpY_i+nc,tmpZ_i+nc);
    ...
    if mod(t,lenX-1)==0,fprintf('\n');end
    
end
masterVals(isnan(masterVals))=0;
masterVals = gather(masterVals);
masterInd = gather(masterInd);
obj.measuredK = accumarray(masterInd,masterVals,[l2^3 1],@mean);
%}
%% reshape and make Hermitian matrix
obj.measuredK = reshape(obj.measuredK,[l2 l2 l2]);
