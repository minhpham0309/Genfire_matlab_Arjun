...
function obj = fillInFourierGrid_DFT2(obj)
...
if obj.DFT_CentroSymmetricity
    ...
    k1 = single((-1*ceil((n1-1)/2):1:floor((n1-1)/2)) / n1);
    k2 = single((-1*ceil((n2-1)/2):1:floor((n2-1)/2)) / n2);
    k3 = k1;
end
...
for p=1:size(obj.InputProjections,3)
    ...
    for i=1:cutloopnum
        ...
        if obj.DFT_doGPU
            ...
            %disp([size(G_CP_plane), size(curr_proj),length(Fpoints)]);
        end
